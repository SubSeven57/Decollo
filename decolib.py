...
def numix(answer):
    ppa = "ppa:numix/ppa"
    program = "numix-gtk-theme numix-icon-theme-circle numix-icon-theme-shine"
    ppaInstall(ppa, program, answer)
...
# Installs VLC browser plugin and removes Totem
def vlcPlugin(answer):
    x = promptInstall(answer, 'sudo apt-get install -y browser-plugin-vlc && sudo apt-get remove -y totem && sudo apt-get autoremove')
    clear()
